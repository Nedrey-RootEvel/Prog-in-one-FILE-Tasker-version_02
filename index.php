...
class Storage {
    ...
    public function checkTabPermission($user, $tab, $action) {
        if (!$user) return false;
        if ($user['role'] === 'admin') return true;
        
        // Для вкладки "Все записи" - просмотр разрешен всем, добавление запрещено всем
        if ($tab['type'] === 'all') {
            if ($action === 'view') return true;
            if ($action === 'add') return false;
            if ($action === 'edit') return $this->checkPermission($user, 'edit');
            if ($action === 'delete') return false;
        }
        
        // Для вкладки "Корзина" - просмотр только админам
        if ($tab['type'] === 'trash') {
            return $user['role'] === 'admin';
        }
        
        // Для пользовательских вкладок - проверяем права вкладки
        ...
        if ($action === 'view') {
            // Если у пользователя есть право просматривать все вкладки
            if ($this->checkPermission($user, 'view_tab_all')) return true;
            return $tab_perms['view'] ?? true;
        }
        
        if ($action === 'add') return $tab_perms['add'] ?? true;
        if ($action === 'edit') return $tab_perms['edit'] ?? true;
        if ($action === 'delete') return $tab_perms['delete'] ?? true;
    ...
    public function getItemsByTab($tab_id, $user = null) {
        ...
        // Проверяем права на просмотр вкладки
        ...
        if ($tab['type'] === 'trash') {
            ...
            // Для вкладки "Все записи" показываем все активные записи
    ...
    public function add($title, $desc, $tab_id = 1) {
        // Проверяем, можно ли добавлять записи во вкладку
        $tab = null;
        foreach ($this->tabs_data as $t) {
            if ($t['id'] == $tab_id) {
                $tab = $t;
...
// Проверяем права на просмотр текущей вкладки
if (!$db->checkTabPermission($current_user, $current_tab_data, 'view')) {
    // Если нет прав, перенаправляем на первую доступную вкладку
    foreach ($tabs as $tab) {
        if ($db->checkTabPermission($current_user, $tab, 'view')) {
            header('Location: ?tab=' . $tab['id']);
            exit;
        }
    }
}
...
$can_add_to_tab = $current_tab_data ? $db->checkTabPermission($current_user, $current_tab_data, 'add') : false;
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-size: 14px; }
        ...
        .btn.tab-permissions { background: #fd7e14; border-color: #dc6b12; }
        .btn.tab-permissions:hover { background: #dc6b12; }
...
<body>
        ...
        <div class="tabs-container">
            <div class="tabs-list">
                <?php foreach ($tabs as $tab): 
                    if (!$db->checkTabPermission($current_user, $tab, 'view')) continue;
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 220px;">ДАТЫ / СТАТУС</th>
                ...
                <tbody>
                    <?php if (!$items): ?>
                    <tr>
                        <td colspan="<?=$current_tab_data && $current_tab_data['type'] == 'all' ? 7 : 6?>" style="padding: 0;">
                ...
                <div class="permissions-group">
                    ...
                    <div class="permission-row">
                        <span class="permission-label">
                            <input type="checkbox" id="tab_perm_view" checked> 👁️ Просмотр записей
                        </span>
                    </div>
                    <div class="permission-row">
                        <span class="permission-label">
    ...
    <script>
        ...
        function addTab() {
            ...
            document.getElementById('tab_perm_view').checked = true;
        ...
        function saveTab(e) {
            ...
            let permissions = {
                view: document.getElementById('tab_perm_view').checked,
        ...
        function exportExcel() {
            ...
            if (isAllTab) {
                data.push(['№', 'Вкладка', 'Заголовок', 'Описание', 'Автор', 'Выполнено', 'Актуально', 'Дата создания', 'Дата изменения', 'Кем изменено']);
            } else {
                data.push(['№', 'Заголовок', 'Описание', 'Автор', 'Выполнено', 'Актуально', 'Дата создания', 'Дата изменения', 'Кем изменено']);
            }
                ...
                if (!row.querySelector('.empty-state')) {
                    ...
                    if (cols.length > 1) {
                        ...
                        if (isAllTab) {
                            ...
                            data.push([num, tabName, title, desc, author, completed, actual, created, updated, updatedBy]);
                        } else {
                            ...
                            let completed = statusCells[0]?.querySelector('input')?.checked ? 'Да' : 'Нет';
                            let actual = statusCells[1]?.querySelector('input')?.checked ? 'Да' : 'Нет';
                            let dates = cols[4]?.querySelector('div:first-child')?.innerHTML || '';
                            let created = dates.split('<span')[0]?.replace('📅', '').trim() || '';
                            let updatedInfo = dates.match(/✏️ (.*?)<\/span>/);
                            let updated = updatedInfo ? updatedInfo[1].split('(')[0].trim() : '';
                            let updatedBy = updatedInfo && updatedInfo[1].includes('(') ? updatedInfo[1].match(/\((.*?)\)/)?.[1] || '' : '';
                            data.push([num, title, desc, author, completed, actual, created, updated, updatedBy]);
                        }
                    }
                }
            });
